                ...
                <div class="card shadow rounded">
                    ...
                    <div class="card-body justify-content-center">
                        <form action="acoes.php" method="POST">
                            <input type="hidden" name="idTransacao"
                                value="<?= $transacao['id'] ?>">
                            <div class="mb-3">
                                ...
                                <select name="txtType" id="txtType"
                                    class="form-select">
                                    <option value="input"
                                        <?= $transacao['type'] == 'input' ? 'selected' : '' ?>>Entrada</option>
                                    <option value="output"
                                        <?= $transacao['type'] == 'output' ? 'selected' : '' ?>>Saída</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="txtValor">Valor</label>
                                <input type="number" name="txtValor"
                                    id="txtValor" class="form-control"
                                    value="<?= $transacao['value'] ?>"
                                    step="0.01">
                            </div>
                            <div class="mb-3">
                                <label for="txtDescricao">Descrição</label>
                                <textarea name="txtDescricao" id="txtDescricao"
                                    class="form-control"
                                    rows="3"><?= $transacao['description'] ?></textarea>
                            </div>
                            <div class="mb-3">
                            ...
                            ?>
                            <button name="edit_transacao" type="submit"
                                class="btn btn-success float-end"><i
                                    class="bi bi-floppy2-fill shadow rounded">
                                    Salvar</i></button>
                        </form>
                    </div>
                </div>
